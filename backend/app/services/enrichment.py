...
def run_ai_enrichment(
    ...
    if settings.OPENAI_API_KEY:
        ...
        system_prompt = (
            ...
            "by invention. internal_corpus.exact_matches are exact evidence. internal_corpus.family_matches "
            "may support only family-safe commercial fields and never variant facts. internal_corpus.comparables "
            "may inform taxonomy, positioning, audience, format and sensory vocabulary only. Never transfer "
            ...
            "When _beautypim_product_understanding is supplied it is the authoritative pre-enrichment "
            "identity and taxonomy contract. Do not reinterpret supplier/legal-entity names as consumer brands, "
            "do not promote source placeholders such as STD, C or BOTH, and do not contradict its category_module. "
            "Generate exactly that one category module. If its module is unknown, generate universal conservative "
            "content only and leave every category-specific module null until identity-first research resolves it. "
            "Exact-GTIN identity evidence outranks inferred title parsing; human overrides still outrank it. "
    ...
    system_prompt = (
        ...
        "_beautypim_product_understanding is the authoritative identity/taxonomy contract: preserve its semantic roles, "
        "never promote STD/C/BOTH placeholders, generate only its category_module, and generate no category-specific module when it is unknown. "
        ...
        "certification, free-from claim, price or compliance status. internal_corpus.family_matches may support only "
        "family-safe commercial fields; internal_corpus.comparables may support only taxonomy, positioning, audience, "
        "format and sensory vocabulary. Never transfer ingredients, claims, identifiers, prices, availability, testing, "
        "regulatory facts or variant identity from family/comparable records. Preserve exact-source conflicts. "
        "Only report a pregnancy ingredient observation when a named retinoid is explicitly present as an INCI item. Never infer retinol from product type, benefits, marketing language, or unrelated oils. Keep any observation factual and make no medical safety conclusion."
    )
